...
from model_io import _nnet2file
...
def _dropout_from_layer(rng, (x,x_NN), p, knn=None):
    ...
    if (x_NN!=None and knn!=None):
        mask_NN = T.extra_ops.repeat(mask,knn,axis=0)   # need to apply same mask over each sample + corresponding NN's
...
class DropoutHiddenLayer(HiddenLayer):
    def __init__(self, rng, input, n_in, n_out, dropout_rate, W=None, b=None,
        ...
        self.output = _dropout_from_layer(self.theano_rng, self.output, p=dropout_rate, knn=knn)
...
#            on_unused_input='warn'
        ...
        if (train_set_NN!=None and gamma!=None and wij_train!=None):
            ...
            train_model_NN = theano.function(
                ...
                givens={
                    self.x: train_set_x,
                    self.y: (train_set_y.flatten()),
                    self.x_NN: train_set_NN, 
                    self.g: gamma_sh,            
                    self.wij: wij_train 
                },
...
#            on_unused_input = 'warn'
    ...
    def build_extract_feat_function(self, output_layer):
        ...
        feat = T.matrix('feat')
        if (output_layer==-1):
            outp = self.layers[output_layer].output
        else:
            outp = self.layers[output_layer].output[0]
    ...
    def write_model_to_raw(self, file_path):
        ...
        if self.dropout_rates!=None:
            _nnet2file(self.layers, filename=file_path, input_factor = self.dropout_rates[0], factor = self.dropout_rates)
        else:
