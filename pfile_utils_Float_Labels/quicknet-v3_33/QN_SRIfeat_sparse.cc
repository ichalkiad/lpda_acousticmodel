const char* QN_SRIfeat_sparse_rcsid =
    "$Header: /u/drspeech/repos/quicknet2/QN_SRIfeat_sparse.cc,v 1.5 2013/10/29 04:16:42 davidj Exp $";
...
void QN_OutFtrStream_SRI_sparse::write_hdr(char* xtra_info) {
  ...
  */
  ...
  pos += sprintf(&hdr_buff[pos],"%7s",sri_hdr_string);
  // the value of 'sri_hdr_len' is defined above as 1024
  pos += sprintf(&hdr_buff[pos],"\n%7u",sri_hdr_len);

  if (ftr_name!=NULL)  {
      pos += sprintf(&hdr_buff[pos],"\nfile_type -s%lu %s",
	      (unsigned long) strlen(sri_sparse_featurefile_type),
	      sri_sparse_featurefile_type);
      pos += sprintf(&hdr_buff[pos],"\nfeature_names -s%lu %s",
  ...
  pos += sprintf(&hdr_buff[pos],"\nsample_count -i %lu",(unsigned long)n_ftrs*frames_this_seg);
  pos += sprintf(&hdr_buff[pos],"\nsample_n_bytes -i 2");
  pos += sprintf(&hdr_buff[pos],"\nchannel_count -i 1");
  pos += sprintf(&hdr_buff[pos],"\nnum_elements -i %lu",(unsigned long) n_ftrs);
  pos += sprintf(&hdr_buff[pos],"\nnum_frames -i %lu",(unsigned long) frames_this_seg);
