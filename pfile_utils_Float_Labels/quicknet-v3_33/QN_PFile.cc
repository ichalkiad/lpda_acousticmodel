...

{
	...
	if (pfile_sent==current_sent)
	{
		...
		if (labs!=NULL)
		{
		    qn_btoh_vf_vf(num_lab_cols,
			     (const float *) &(buffer[first_lab_col].f),
			     (float *) labs);
		    labs += num_lab_cols;
		}
		read_frame();
				      ...
				      const float* labs)
{
	...
	if (num_labs!=0)
	{
	    if (labs!=NULL)
	    {
		qn_htob_vf_vf(num_labs, (const float*) labs,
			       &(buffer[2+num_ftrs].f));
	    	labs += num_labs;
	    }
	    else
		qn_copy_f_vf(num_labs, 0, &(buffer[2+num_ftrs].f));
	}
...
{
     std::cout<<"kjsdsjsjksjjnksf"<<std::endl;
...
void
QN_OutFtrLabStream_PFile::write_labs(size_t frames, const float* labs)
{
     std::cout<<"kjsdsj4sjksjjnksf"<<std::endl;
