...
const char* QN_Logger_Simple_rcsid =
    "$Header: /u/drspeech/repos/quicknet2/QN_Logger_Simple.cc,v 1.14 2013/10/29 04:16:42 davidj Exp $";
#endif
...
QN_Logger_Simple::verror(const char* objname, const char* format, va_list va)
{
    ...
    // make a copy of va just in case we use it below
    va_copy(vc,va);
    vfprintf(err_stream, format, vc);
    va_end(vc);
    fputc('\n', err_stream);
