const char* QN_MLP_ThreadFl3_rcsid =
    "$Header: /u/drspeech/repos/quicknet2/QN_MLP_ThreadFl3.cc,v 1.14 2013/10/29 04:16:42 davidj Exp $";
...
//     }
	...
	for (i = 0; i<n_output; i++)
	{
	    ...
	    sum = *threadp[0];
	    threadp[0] += 1;
	    for (thread = 1; thread<useful_threads; thread++)
	    {
		sum += *threadp[thread];
		threadp[thread] += 1;
	    }
