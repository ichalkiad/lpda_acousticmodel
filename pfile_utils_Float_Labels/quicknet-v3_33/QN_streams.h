...
public:
    ...
    virtual void write_labs(size_t cnt, const float* labs) = 0;
};
...
public:
    ...
    // If the return value is less than "cnt", we have reached the end of the
    // segment.
    virtual size_t read_labs(size_t cnt, float* labs) = 0;
    
     // A strided version of the read_labs function.  Initially, not all
    // derived classes will implement this, so provide a default version.
    // If stride==QN_ALL, stride is same as num_labs().
    virtual size_t read_labs(size_t cnt, float* labs, size_t stride);
...
inline size_t
QN_InLabStream::read_labs(size_t cnt, float* labs, size_t stride)
...
public:
    ...
    // If the return value is less than "cnt", we have reached the end of the
    // segment.
    virtual size_t read_ftrslabs(size_t cnt, float* ftrs, float* labs) = 0;
