const char* QN_trn_rcsid =
    "$Header: /u/drspeech/repos/quicknet2/QN_trn.cc,v 1.27 2013/10/29 04:16:43 davidj Exp $";
...
QN_HardSentTrainer::cv_epoch()
{
	...
	if (ftr_count<bunch_size) // Check if at end of segment.
	{
	    ...
	    if (ftr_segid!=QN_SEGID_UNKNOWN
		&& lab_segid!=QN_SEGID_UNKNOWN
		&& ftr_segid!=lab_segid)
	    {
		clog.error("Hard training CV segids do not match");
	    }
...
QN_HardSentTrainer::train_epoch()
{
	...
	if (ftr_count<bunch_size) // Check if at end of segment.
	{
		...
		&& ftr_segid!=lab_segid)
	    {
		clog.error("Hard training train segids do not match");
	    }
	    if (ftr_segid==QN_SEGID_BAD)
		break;
	    else
...
QN_SoftSentTrainer::cv_epoch()
{
	...
	if (ftr_count<bunch_size) // Check if at end of segment.
	{
	    ...
	    if (ftr_segid!=QN_SEGID_UNKNOWN
		&& targ_segid!=QN_SEGID_UNKNOWN
		&& ftr_segid!=targ_segid)
	    {
		clog.error("Soft training CV segids do not match");
	    }
	    if (ftr_segid==QN_SEGID_BAD)
		break;
	    else
...
QN_SoftSentTrainer::train_epoch()
{
	...
	if (ftr_count<bunch_size) // Check if at end of segment.
	{
		...
		&& ftr_segid!=targ_segid)
	    {
		clog.error("Soft training train segids do not match");
	    }
	    if (ftr_segid==QN_SEGID_BAD)
		break;
	    else
