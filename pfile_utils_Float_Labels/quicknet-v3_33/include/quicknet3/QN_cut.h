// -*- C++ -*-
// $Header: /u/drspeech/repos/quicknet2/QN_cut.h,v 1.6 2013/10/29 04:02:31 davidj Exp $
...
// Some private functions.
    ...
    // Make a given cut the current one, returning whether reseek needed.
    virtual int select_cut(CutSelector new_cut);
    // Reseek after a select_cut.
    virtual void reseek();
};
...
// Some private functions.
    ...
    // Actually do the work of the set_pos() calls.
    virtual QN_SegID set_pos(CutSelector cut, size_t segno, size_t frameno);
    // Make a given cut the current one, returning whether a reseek needed.
    virtual int select_cut(CutSelector new_cut);
};
