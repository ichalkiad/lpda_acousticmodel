			     ...
			     const size_t n_labs)
{
  if (buf_size+1 > buf_array_length) {
    ...
    float* lab_tmp = new float [buf_array_length * n_labs];
    ::memcpy(ftr_tmp,ftr_buf,sizeof(float)*buf_size*n_ftrs);
    ::memcpy(lab_tmp,lab_buf,sizeof(float)*buf_size*n_labs);
    delete ftr_buf;
    delete lab_buf;
    ftr_buf = ftr_tmp;
    lab_buf = lab_tmp;
	     ...
	     const bool binary)
{
    ...
    float *lab_buf = new float[buf_array_length * n_labs];
    size_t n_frames=0;
    size_t sent_no=0;
