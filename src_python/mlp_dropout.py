...
def _dropout_from_layer(rng, (layer,layer_NN), p):
    ...
    mask_NN = srng.binomial(n=1, p=1-p, size=layer_NN.shape)
    output = (layer * T.cast(mask, theano.config.floatX),
                layer_NN * T.cast(mask_NN, theano.config.floatX))
    ...
    def __init__(self, rng, input, n_in, hidden_layers_sizes, n_out,dropout_rates=None,activations=None):
        ...
        for i in xrange(self.n_layers):
            ...
            if i == 0:
                ...
                if dropout_rates!=None:
                    dropout_layer_input = self.dropout_layers[i-1].output
            ...
            # dropout hidden layers for training network
            if dropout_rates!=None:
            ...
            # hidden layers for test network
            if dropout_rates!=None:
                sigmoid_layer = HiddenLayer(
                    ...
                    n_in=input_size,
                    n_out=hidden_layers_sizes[i],
                    W = dropout_layer.W*(1 - dropout_rates[i]),
                    b = dropout_layer.b,
                    activation=activation
                )
            else:
        ...
        if dropout_rates!=None:
            ...
            self.logRegressionLayer = LogisticRegression(
                ...
                n_in=hidden_layers_sizes[-1],
                n_out=n_out,
                W=self.dropout_logRegr_layer.W * (1 - dropout_rates[-1]),
                b=self.dropout_logRegr_layer.b
            )
        else:
            self.logRegressionLayer = LogisticRegression(
                input=self.sigmoid_layers[-1].output,
        ...
        # to be used in validation and test evaluations
...
def test_mlp(learning_rate_init=0.04,
...
#            save_parameters(classifier.params,save_file) 
            ...
            learning_rate = decay_learning_rate(epoch_counter)
