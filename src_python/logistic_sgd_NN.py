...
#        self.y_pred_NN = T.argmax(self.p_y_given_x_NN, axis=1)
        # end-snippet-1
    ...
    def negative_log_likelihood(self, y, gamma=None, wij=None, knn=None):
        ...
        #Cross entropy cost, equivalent to negative log-likelihood
        y_one_hot = T.extra_ops.to_one_hot(y,self.p_y_given_x.shape[1])
        x_entr = T.nnet.categorical_crossentropy(self.p_y_given_x,y_one_hot)
        ...
        if (gamma!=None) and (wij!=None) and (self.p_y_given_x_NN!=None) and (knn!=None):
            ...
            manifold_term = T.sum( T.reshape( ((2*gamma/knn) * T.sum((y_repeated - self.p_y_given_x_NN)**2,1) * wij), (self.p_y_given_x.shape[0],knn)),1)
            return T.mean(x_entr + manifold_term)
        else:
